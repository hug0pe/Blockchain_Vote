...
contract SimpleVotingSystem is AccessControl {
    ...
    event FundsSent(uint candidateId, uint amount);
    ...
    function setWorkflowStatus(WorkflowStatus _status)
        external
        onlyRole(ADMIN_ROLE)
    {
        status = _status;
    ...
    function addCandidate(string memory _name) public onlyRole(ADMIN_ROLE) onlyStatus(WorkflowStatus.REGISTER_CANDIDATES)
    {
    ...
    function vote(uint _candidateId) public onlyStatus(WorkflowStatus.VOTE) 
    {
    ...
    function fundCandidate(uint _candidateId) external payable onlyRole(FOUNDER_ROLE) onlyStatus(WorkflowStatus.FOUND_CANDIDATES)
    {
    ...
    function getWinner() external view onlyStatus(WorkflowStatus.COMPLETED)
